...
def _sync_decision_cross_ranks(
    ...
    if (
        ...
        with no_dispatch(), unset_fake_temporarily():
            ...
            saved_ops_with_sizes: dict[int, str] = {}
            for idx, saved_ops_names in enumerate(saved_ops_names_all_ranks):
                ...
                saved_size = 0
                saved_ops_with_sizes[idx] = ""
                for node in saved_nodes:
                    size_of_node = _size_of(node)
                    saved_size += size_of_node
                    saved_ops_with_sizes[idx] += f"{node.name}:{size_of_node},"
                saved_ops_with_sizes[idx] += f"total size:{saved_size}\n"
            ...
            sync_decision_cross_ranks_str = (
                f"picked_rank_idx={picked_rank_idx}, {saved_ops_with_sizes}"
            )
            trace_structured(
                "artifact",
                metadata_fn=lambda: {
